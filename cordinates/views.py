...
class CordinatesView(APIView):
    ...
    def post(self, request):
        ...
        try:
            response = shortest_distance(casted_cordinates)
        except TypeError:
            return Response(data="Invalid Request", status=status.HTTP_400_BAD_REQUEST)
    ...
    def get(self, request):
        return Response(data=Cordinate.objects.all().values(), status=status.HTTP_200_OK)


def shortest_distance(cordinates: list):
    ...
    save_cordinate(result, cordinates)
    return result


def distance_calc(cordinate: tuple, compare_cord: tuple):
...
def build_result(result: dict, cordinate_pair: str, distance: float):
...
def save_cordinate(result: dict, cordinates: list):
    cord = Cordinate()
